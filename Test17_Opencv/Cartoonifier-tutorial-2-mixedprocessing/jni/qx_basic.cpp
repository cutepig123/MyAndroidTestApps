...
//}
//void qx_timer::time_display(char *disp,int nr_frame){ printf("Running time (%s) is: [%5.5f] Seconds (%d iterations).\n",disp,stop()/nr_frame,nr_frame);}
//void qx_timer::fps_display(char *disp,int nr_frame){ printf("Running time (%s) is: [%5.5f] frame per second (%d iterations).\n",disp,(double)nr_frame/stop(),nr_frame);}
//
...
void boxcar_sliding_window_x(UCHAR*out,UCHAR*in,int h,int w,int radius)
{
    float scale = 1.0f / (2*radius+1);    
    for (int y = 0; y < h; y++) {    
        float t;
        ...
        out[y*w] = UCHAR(t * scale+0.5);
        for(int x = 1; x < radius+1; x++) {
            int c = y*w+x;
            t += in[c+radius];
            t -= in[y*w];
            out[c] = UCHAR(t * scale+0.5);
        }        
        // main loop
        for(int x = radius+1; x < w-radius; x++) {
            int c = y*w+x;
...
void boxcar_sliding_window_y(UCHAR*out,UCHAR*in,int h,int w,int radius)
{
    float scale = 1.0f / (2*radius+1);    
    for (int x = 0; x < w; x++) 
	{    
...
void boxcar_sliding_window(UCHAR**out,UCHAR**in,UCHAR**temp,int h,int w,int radius)
{
	int min_hw=min(h,w);
	if(radius<min_hw)
	{
...
void qx_specular_free_image(UCHAR ***image_specular_free,UCHAR ***image_normalized,float **diffuse_chromaticity_max,int h,int w)
{
	int y,x;
	UCHAR *image_specular_free_x,*image_normalized_x; float *diffuse_chromaticity_max_x;
	UCHAR r,g,b; double imax,isum; float rf,gf,bf,c,t0,t1,t2,t3,diffuse,specular;
		...
		for(x=0;x<w;x++)
		{
			...
			if(t1>0)
			{
				...
				else
				{
					...
					*image_specular_free_x++=UCHAR(rf+0.5f);
					*image_specular_free_x++=UCHAR(gf+0.5f);
					*image_specular_free_x++=UCHAR(bf+0.5f);
				}
			}
			else
			{
...
void depth_best_cost(UCHAR**depth,double***evidence,int h,int w,int nr_planes)
{
...
void qx_detect_occlusion_left_right(UCHAR**mask_left,UCHAR**depth_left,UCHAR**depth_right,int h,int w,int nr_plane)
{
...
{
	FILE *file_in; char str[300]; int i;
