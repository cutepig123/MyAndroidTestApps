...
void cartoonifyImage(Mat const& srcColor, Mat &dst, bool sketchMode, bool alienMode, bool evilMode, int debugType)
{
	...
	unsigned char* pixelPtr = (unsigned char*)smallImg.data;
	int cn = smallImg.channels();

		unsigned char***texture=qx_allocu_3(smallImg.rows,smallImg.cols,3);
		double***image=qx_allocd_3(smallImg.rows,smallImg.cols,3);
		double***image_filtered=qx_allocd_3(smallImg.rows,smallImg.cols,3);
		double***temp=qx_allocd_3(sh,sw,3);
		double***temp_2=qx_allocd_3(2,sw,3);

for(int i=0; i<smallImg.rows; i++){
   for(int j=0; j<smallImg.cols; j++){
           texture[i][j][2]=smallImg.data[smallImg.step[0]*i + smallImg.step[1]* j + 0] ;	//b
           texture[i][j][1]=smallImg.data[smallImg.step[0]*i + smallImg.step[1]* j + 1] ;	//g
           texture[i][j][0]=smallImg.data[smallImg.step[0]*i + smallImg.step[1]* j + 2];	//r
		   image[i][j][0]=texture[i][j][0];
		   image[i][j][1]=texture[i][j][1];
		   image[i][j][2]=texture[i][j][2];
    }
} 
...
//for(int m=0; m<10; m++)
	qx_gradient_domain_recursive_bilateral_filter(image_filtered,image,texture,0.03,0.1,sh,sw,temp,temp_2);

	for(int i = 0; i < smallImg.rows; i++)
{
    for(int j = 0; j < smallImg.cols; j++)
    {
        texture[i][j][0]=image_filtered[i][j][0];	//r
		texture[i][j][1]=image_filtered[i][j][1];	//g
		texture[i][j][2]=image_filtered[i][j][2];	//b
		
          smallImg.data[smallImg.step[0]*i + smallImg.step[1]* j + 0]=texture[i][j][2];	//b
           smallImg.data[smallImg.step[0]*i + smallImg.step[1]* j + 1]=texture[i][j][1] ;	//g
           smallImg.data[smallImg.step[0]*i + smallImg.step[1]* j + 2]=texture[i][j][0];	//r

	}
}
	//show textture img for debugging
	/*IplImage* photoImg=cvCreateImage(cvSize(sw,sh),IPL_DEPTH_8U,cn);
			for( int m = 0; m < sh; m++ )
		{
		   for (int n = 0; n < sw; n++ )
		   {
			  photoImg->imageData[photoImg->widthStep * m + n * 3]=texture[m][n][2];
			  photoImg->imageData[photoImg->widthStep * m + n * 3 + 1]= texture[m][n][1];
			  photoImg->imageData[photoImg->widthStep * m + n * 3 + 2]=texture[m][n][0];
		   }
		 }

		cvNamedWindow("Window1");
		cvShowImage("Window1",photoImg);
